...
def add_log_to_csv(user_id, browser_id, website_id, status):
    log = db.query('SELECT website_id, site_title, jahia, wordpress, os, name, version, date, user_id, first_name, last_name, status from full_logs WHERE user_id=' + str(user_id) + ' AND browser_id=' + str(browser_id) + ' AND website_id=' + str(website_id) + ' AND status="' + status + '";').list()
    if log:
        log = log[0]
        row_to_write = (str(log.website_id) + "," + log.site_title + "," + log.name + "," + log.jahia + "," + log.wordpress + "," + log.os + "," + log.name + "," + str(log.version) + "," + datetime.datetime.fromtimestamp(log.date).strftime('%Y-%m-%d_%H:%M:%S') + "," + str(log.user_id) + "," + log.first_name + "," + log.last_name + "," + log.status).encode('utf-8')
